...
public class GenServiceImpl implements IGenService
{
    ...
    public byte[] generatorCode(String tableName, Map<String,String[]> columnMap)
    {
        ...
        for(ColumnInfo c : columns){
            String[] ss = columnMap.get(c.getAttrname());
        ...
        // 生成代码
        generatorCode(table, columns,columnMap, zip);
        IOUtils.closeQuietly(zip);
        return outputStream.toByteArray();
    }


    public byte[] generatorCode(TableInfo.GenStyle codeStyle, String tableName, Map<String,String[]> params){
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        ZipOutputStream zip = new ZipOutputStream(outputStream);
        // 查询表信息
        TableInfo table = genMapper.selectTableByName(tableName);
        ...
        // 生成代码
        generatorAdvCodeOnlyWeb(GenUtils.getParam(params,"tmpl"),table,table.getColumns(),params,zip);
        IOUtils.closeQuietly(zip);
        return outputStream.toByteArray();
    }
    /**
    ...
    public TableInfo getTableInfo(String tableName){
        ...
        // 列信息
        table.setColumns(GenUtils.transColums(columns,null));
        // 设置主键
     ...
     * 生成代码
     */
    public void generatorCode(TableInfo table, List<ColumnInfo> columns, Map<String,String[]> params, ZipOutputStream zip)
    {
        // 表名转换成Java属性名
        String className = GenUtils.tableToJava(table.getTableName());
        table.setClassName(className);
    ...
    public void generatorAdvCodeOnlyWeb(String tmplName,TableInfo table, List<ColumnInfo> columns, Map<String,String[]> params, ZipOutputStream zip)
    {
    ...
    private Map generatorAdvCodeOnlyWeb(String tmplName,TableInfo table, List<ColumnInfo> columns, Map<String,String[]> params)
    {
    ...
    public Map previewCode(TableInfo.GenStyle codeStyle, String tableName, Map<String,String[]> params){
        TableInfo table = getTableInfo(tableName);
        if(codeStyle == TableInfo.GenStyle.Web){
            return generatorAdvCodeOnlyWeb(GenUtils.getParam(params,"tmpl"),table,table.getColumns(),params);
        }
        return null;
    }
}
